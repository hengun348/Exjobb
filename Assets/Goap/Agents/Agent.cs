...
public class Agent: MonoBehaviour
{
	...
	void Start(){
		...
		//wMemory.setFact("Wood", new WorkingMemoryValue(new Vector3(3.0f,0.0f,10.0f)));
		//BlackBoard.Instance.setFact("stonePosition", new WorkingMemoryValue(new Vector3(-5.0f,0.0f,0.0f)));
	...
	void Update(){
		...
		}else if(actionIsDone && (planIndex == plan.Count))
		{
			...
			isAvailable = true;
			plan.Clear();
			gameObject.renderer.material.color = new Color(0.0f, 1.0f, 0.0f, 1.0f);
	...
	void OnTriggerEnter(Collider other) { 
		if (other.gameObject.tag == "StoneSource") {
			//Debug.Log("Yay!"); 
			wMemory.setFact("Stone", new WorkingMemoryValue(other.transform.position)); 
		} else if(other.gameObject.tag == "WoodSource"){
		
			wMemory.setFact("Wood", new WorkingMemoryValue(other.transform.position)); 
		}
	}
