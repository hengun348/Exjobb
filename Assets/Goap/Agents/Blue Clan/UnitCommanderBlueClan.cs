...
[AddComponentMenu("Commanders/UnitCommanderBlueClan")]

public class UnitCommanderBlueClan: MonoBehaviour
{
	...
	{	
		...
		for(int i = 0; i < 1; i++)
		{
			...
			yield return StartCoroutine(AddAgent(color));
		}
		...
		{
			//AddAgent(color, agentGroup.transform.position);
			yield return StartCoroutine(AddAgent("Red"));
		}
		
		for(int i = 0; i < 1; i++)
		{
			//AddAgent(color, agentGroup.transform.position);
			yield return StartCoroutine(AddAgent("Yellow"));
		}
	}
