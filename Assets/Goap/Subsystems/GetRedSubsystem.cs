...
public class GetRedSubsystem: MonoBehaviour{
	...
	IEnumerator Start(){
		...
		yield return StartCoroutine(FindRed());
		Destroy(gameObject);
	}
	...
	IEnumerator FindRed(){
		while(actionIsDone == false){	
			if(collected)
			{
				...
				((Agent2)agentComponent).getTarget().transform.position = housePosition; //will be replaced with houseposition
			...
			{
				if(((Agent2)agentComponent).getWMemory().getFact("Red").Count > 0)
				{
					moveToPosition = (Vector3)((Agent2)agentComponent.GetComponent("Agent2")).getWMemory().getFact("Red")[0].getFactValue();
